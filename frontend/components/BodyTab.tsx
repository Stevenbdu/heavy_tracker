...
export function BodyTab() {
  ...
  const handleSave = async () => {
    ...
    if (Object.keys(payload).length === 0) { infoAlert('Vide', 'Remplis au moins une mesure'); return; }
    setSaving(true);
    try {
  ...
  const chartData = useMemo(() =>
    metrics.filter((e) => e[selectedMetric] != null).slice(0, 12).reverse().map((e) => ({
  ...
  return (
    <View style={{ flex: 1 }}>
      <View style={s.header}>
        <View>
          <Text style={s.screenTitle}>Corps</Text>
          <Text style={s.screenSub}>Suivi des mensurations</Text>
        </View>
      ...
      >
        {metrics.length === 0 ? (
          <View style={s.emptyState}>
            ...
            <Text style={s.emptyTitle}>Aucune mensuration</Text>
            <Text style={s.emptySub}>Ajoute ta première entrée pour commencer le suivi</Text>
            <TouchableOpacity style={s.emptyBtn} onPress={openModal} activeOpacity={0.8}>
              <Feather name="plus" size={16} color={colors.accentText} />
              <Text style={s.emptyBtnText}>Ajouter une mensuration</Text>
            </TouchableOpacity>
          </View>
        ) : (
          <>
            <View style={s.section}>
              <Text style={s.sectionTitle}>DERNIÈRES MESURES</Text>
              <View style={s.metricsGrid}>
                {METRICS.filter((m) => latest[m.key] != null).map((m) => {
                  const entry = latest[m.key]!;
                  return (
                    <View key={m.key} style={[s.metricCard, { borderColor: m.color + '40' }]}>
                      <Text style={s.metricCardLabel}>{m.label}</Text>
                      <Text style={[s.metricCardValue, { color: m.color }]}>
                        {entry.value} <Text style={s.metricCardUnit}>{m.unit}</Text>
                      </Text>
                      {entry.delta != null && entry.delta !== 0 && (
                        <Text style={[s.metricCardDelta, { color: entry.delta < 0 ? colors.accent : colors.danger }]}>
            ...
            {availableMetrics.length > 0 && (
              <View style={s.section}>
                <Text style={s.sectionTitle}>ÉVOLUTION</Text>
                <ScrollView horizontal showsHorizontalScrollIndicator={false}>
                  <View style={{ flexDirection: 'row', gap: spacing.xs, paddingBottom: spacing.sm }}>
                    {availableMetrics.map((m) => (
                      <TouchableOpacity
                        key={m.key}
                        style={[s.chip, selectedMetric === m.key && { backgroundColor: m.color + '22', borderColor: m.color }]}
                        onPress={() => setSelectedMetric(m.key)}
                        activeOpacity={0.75}
                      >
                        <Text style={[s.chipText, selectedMetric === m.key && { color: m.color }]}>{m.label}</Text>
                      </TouchableOpacity>
                    ))}
                  </View>
                </ScrollView>
                <View style={s.chartCard}>
                  <View style={s.chartCardHeader}>
                    <Text style={s.chartCardTitle}>{selectedDef.label}</Text>
                    {latest[selectedMetric] && (
                      <Text style={[s.chartCardCurrent, { color: selectedDef.color }]}>
                  ...
                  {chartData.length >= 2 && (() => {
                    ...
                    return (
                      <Text style={[s.chartDelta, { color: isGood ? colors.accent : colors.danger }]}>
                  ...
                  <View style={s.historyLeft}>
                    ...
                    <View style={s.historyValues}>
                      {METRICS.filter((m) => entry[m.key] != null).map((m) => (
                        <Text key={m.key} style={[s.historyValue, { color: m.color }]}>
          ...
          <View style={s.modalOverlay}>
            ...
            <View style={s.modalSheet}>
              ...
              <View style={s.modalTitleRow}>
                <Text style={s.modalTitle}>Nouvelle mensuration</Text>
                <TouchableOpacity onPress={() => setModalOpen(false)} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
                  <Feather name="x" size={20} color={colors.textMuted} />
                </TouchableOpacity>
              </View>
              <ScrollView showsVerticalScrollIndicator={false} style={{ maxHeight: 420 }}>
                <Text style={s.formGroupLabel}>CORPS</Text>
                <View style={s.formRow}>
                  <View style={{ flex: 1 }}>
                    <Text style={s.fieldLabel}>POIDS (kg)</Text>
                    <TextInput style={s.fieldInput} keyboardType="decimal-pad" placeholder="75.5" placeholderTextColor={colors.textMuted} value={form.weight ?? ''} onChangeText={(v) => setForm((f) => ({ ...f, weight: v }))} />
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text style={s.fieldLabel}>TAILLE (cm)</Text>
                    <TextInput style={s.fieldInput} keyboardType="decimal-pad" placeholder="178" placeholderTextColor={colors.textMuted} value={form.height ?? ''} onChangeText={(v) => setForm((f) => ({ ...f, height: v }))} />
                  </View>
                </View>
                <Text style={s.formGroupLabel}>BUSTE</Text>
                <View style={s.formRow}>
                  <View style={{ flex: 1 }}>
                    <Text style={s.fieldLabel}>POITRINE (cm)</Text>
                    <TextInput style={s.fieldInput} keyboardType="decimal-pad" placeholder="95" placeholderTextColor={colors.textMuted} value={form.chest ?? ''} onChangeText={(v) => setForm((f) => ({ ...f, chest: v }))} />
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text style={s.fieldLabel}>ABDOMEN (cm)</Text>
                    <TextInput style={s.fieldInput} keyboardType="decimal-pad" placeholder="82" placeholderTextColor={colors.textMuted} value={form.waist ?? ''} onChangeText={(v) => setForm((f) => ({ ...f, waist: v }))} />
                  </View>
                </View>
                <Text style={s.formGroupLabel}>BAS DU CORPS</Text>
                <View style={s.formRow}>
                  <View style={{ flex: 1 }}>
                    <Text style={s.fieldLabel}>HANCHES (cm)</Text>
                    <TextInput style={s.fieldInput} keyboardType="decimal-pad" placeholder="95" placeholderTextColor={colors.textMuted} value={form.hips ?? ''} onChangeText={(v) => setForm((f) => ({ ...f, hips: v }))} />
                  </View>
                </View>
                <View style={s.formRow}>
                  <View style={{ flex: 1 }}>
                    <Text style={s.fieldLabel}>CUISSE D. (cm)</Text>
                    <TextInput style={s.fieldInput} keyboardType="decimal-pad" placeholder="58" placeholderTextColor={colors.textMuted} value={form.thighR ?? ''} onChangeText={(v) => setForm((f) => ({ ...f, thighR: v }))} />
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text style={s.fieldLabel}>CUISSE G. (cm)</Text>
                    <TextInput style={s.fieldInput} keyboardType="decimal-pad" placeholder="58" placeholderTextColor={colors.textMuted} value={form.thighL ?? ''} onChangeText={(v) => setForm((f) => ({ ...f, thighL: v }))} />
                  </View>
                </View>
                <Text style={s.formGroupLabel}>BRAS</Text>
                <View style={s.formRow}>
                  <View style={{ flex: 1 }}>
                    <Text style={s.fieldLabel}>BRAS D. (cm)</Text>
                    <TextInput style={s.fieldInput} keyboardType="decimal-pad" placeholder="36" placeholderTextColor={colors.textMuted} value={form.armR ?? ''} onChangeText={(v) => setForm((f) => ({ ...f, armR: v }))} />
                  </View>
                  <View style={{ flex: 1 }}>
                    <Text style={s.fieldLabel}>BRAS G. (cm)</Text>
                    <TextInput style={s.fieldInput} keyboardType="decimal-pad" placeholder="36" placeholderTextColor={colors.textMuted} value={form.armL ?? ''} onChangeText={(v) => setForm((f) => ({ ...f, armL: v }))} />
                  </View>
                </View>
                <View style={{ height: spacing.md }} />
              </ScrollView>
              <TouchableOpacity style={[s.saveBtn, saving && { opacity: 0.6 }]} onPress={handleSave} disabled={saving} activeOpacity={0.8}>
                {saving ? <ActivityIndicator color={colors.accentText} /> : <Text style={s.saveBtnText}>Enregistrer</Text>}
              </TouchableOpacity>
            </View>
          </View>
